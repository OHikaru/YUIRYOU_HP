...
export default async function HomePage() {
  ...
  return (
    <>
          ...
          <div className="comparison-table" role="table" aria-label="比較表">
            ...
            {comparisonRows.map((row) => (
              <div className="comparison-table__row" role="row" key={row.label}>
                <span>{row.label}</span>
                {row.values.map((value) => <span key={`${row.label}-${value}`}>{value}</span>)}
              </div>
            ))}
          </div>
      ...
      <section id="pricing" className="section section--deep">
        <div className="shell">
          ...
          <div className="pricing-guardrails">
            <p>法人向けサービスです。</p>
            <p>患者個人の相談は対象外です。</p>
      ...
      <section className="section section--cta">
        <div className="shell cta-banner">
          <div>
            ...
            <h2>まずは無料相談で、何をどこまで外部化すべきか整理してください</h2>
            <p>監修だけで足りるのか、研究設計まで必要なのか、AIや発信まで含めるべきかをヒアリングし、診断パックまたは月額プランにつなげます。</p>
          </div>
          <div className="hero-actions">
            <Link href="/contact" className="button button--solid">無料相談を申し込む</Link>
            <Link href="/insights" className="button button--ghost button--light">インサイトを見る</Link>
          </div>
        </div>
      </section>
    </>
