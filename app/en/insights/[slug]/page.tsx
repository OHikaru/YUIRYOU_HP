...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return buildMetadata({
    ...
    path: `/en/insights/${localized.slug}`,
    locale,
  });
}

export default async function EnglishInsightDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  ...
  const articleSchema = {
    ...
    author: { "@type": "Person", name: author?.name ?? siteConfig.brandNameEn, description: author?.role ?? copy.fallbackRole },
    publisher: { "@type": "Organization", name: siteConfig.brandNameEn, url: siteConfig.siteUrl },
    mainEntityOfPage: `${siteConfig.siteUrl}/en/insights/${localizedArticle.slug}`,
  };
  ...
  return (
    <>
      ...
      <article>
        <PageHeroWithImage
          items={[{ href: "/en/insights", label: copy.listLabel }, { href: `/en/insights/${localizedArticle.slug}`, label: localizedArticle.title }]}
        ...
        <section className="section article-section">
          <div className="shell narrow-shell article-stack">
            <div className="article-panel article-panel--summary">
              <SectionLead eyebrow="Summary" title={copy.summaryTitle} />
              <p>{localizedArticle.summary}</p>
            </div>

            <div className="article-panel article-panel--lead">
              <SectionLead eyebrow="TL;DR" title={copy.tldrTitle} />
            ...
            <div className="article-panel article-panel--author">
              ...
              <h2>{author?.name ?? siteConfig.brandNameEn}</h2>
              <p className="role-label">{author?.role ?? copy.fallbackRole}</p>
              <p>{author?.bio ?? copy.fallbackBio}</p>
              <div className="hero-actions">
                <Link href="/en/team" className="button button--ghost">{copy.viewTeamLabel}</Link>
                <Link href="/en/contact" className="button button--solid">Book a consultation</Link>
              </div>
            </div>
                ...
                <div className="related-articles">
                  {relatedArticles.map((related) => (
                    <article key={related.slug} className="related-article-card">
                      ...
                      <h3><Link href={`/en/insights/${related.slug}`}>{related.title}</Link></h3>
                      <p>{related.summary}</p>
                    </article>
                  ))}
                </div>
